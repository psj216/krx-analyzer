...
if st.button("점수 계산 / 업데이트", use_container_width=True):
    try:
        ...
        status_box.info("점수 계산 중... 0/0")

        def _cb(done, total):
            progress.progress(done / max(total, 1), text=f"점수 계산 중... {done}/{total}")
            status_box.info(f"점수 계산 중... {done}/{total}")
        ...
        progress.progress(1.0, text=f"점수 계산 완료... {len(result)}/{candidate_limit}")
...
if isinstance(topdf, pd.DataFrame) and not topdf.empty:
    ...
    cols = ["Code", "Name", "Market", "Score", "Grade", "Trend", "Momentum", "Breakout", "Flow", "Vol", "IchiSc", "Close"]
...
def run_once():
    ...
    m1.metric("Score", f"{score_result['Score']} ({score_result['Grade']})")
    m2.metric("Trend", score_result["Trend"])
    m3.metric("Momentum", score_result["Momentum"])
    m4.metric("Breakout", breakout_display)
    m5.metric("Flow", flow_display)
    m6.metric("Volume / Ichi", f"{vol_display} / {ichi_display}")

    st.subheader("유형별 점수 상세")
    section_rows = [
        {"구분": "추세 점수", "점수": score_result["Trend"], "설명": "중장기 추세 방향과 안정성"},
        {"구분": "모멘텀 점수", "점수": score_result["Momentum"], "설명": "단기 힘, RSI, MACD, 5일선 흐름"},
        {"구분": "돌파 점수", "점수": score_result["Breakout"], "설명": "장대양봉, 고점 돌파, 볼린저/거래량 동반"},
        ...
        {"구분": "거래량 점수", "점수": vol_display, "설명": f"오늘 거래량/20일 평균 비율 {score_result.get('VolumeRatio'):.2f}배" if score_result.get("VolumeRatio") is not None else "거래량 비율 계산 불가"},
    ...
    with st.expander("추세 점수", expanded=True):
        for item in tech.get("reasons_trend", []):
            st.markdown(f"- {item}")
    with st.expander("모멘텀 점수", expanded=True):
        for item in tech.get("reasons_momentum", []):
            st.markdown(f"- {item}")
    with st.expander("돌파 점수", expanded=True):
        for item in tech.get("reasons_breakout", []):
            st.markdown(f"- {item}")
    with st.expander("수급 점수", expanded=True):
    ...
    with st.expander("거래량 점수", expanded=True):
        if score_result.get("VolumeRatio") is not None:
            st.markdown(f"- 오늘 거래량이 20일 평균 대비 {score_result['VolumeRatio']:.2f}배로 반영됨 → 거래량 점수 {score_result['Vol']}")
        else:
            st.markdown("- 거래량 데이터가 부족해 기본 점수 처리")
    with st.expander("일목 점수", expanded=True):
        for item in score_result.get("IchiDetail", []):
            st.markdown(f"- {item}")

    warn_lines = []
    if score_result.get("Overheat", 0) >= 50:
        warn_lines.append(f"과열 경고: RSI/볼린저/윗꼬리 기준 추격 주의 ({score_result['Overheat']})")
    if score_result.get("LiqRisk", 0) >= 40:
        warn_lines.append(f"유동성 경고: 평균 거래대금 {_fmt_eok(score_result.get('AvgTurnover20'))} 수준 확인 필요 ({score_result['LiqRisk']})")
    if warn_lines:
        st.subheader("보조 경고")
