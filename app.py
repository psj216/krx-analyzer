...
def rank_top_scores(all_syms,universe="ALL",limit=1000,lookback=400,regime="sideways"):
    ...
    if len(syms)>limit:
        # 코드 순서 bias 제거: 당일 고정 시드로 무작위 샘플링
        daily_seed=int(datetime.now(KR_TZ).strftime("%Y%m%d"))%2147483647
        syms=syms.sample(limit,random_state=daily_seed).reset_index(drop=True)
...
def run_once():
    ...
    rt=get_realtime_price(code)  # 표시 전용 — 점수에는 미반영 (랭킹과 기준 통일)
    ...
    _fr=w["trend"]*sc["trend_raw"]+w["momentum"]*sc["momentum_raw"]+ \
        w["flow"]*flow_sc+w["volume"]*vol_sc+w["ichi"]*ichi_sc
    _fr=max(0.0, _fr - sc["overheat_score"]*0.12 - sc["liquidity_risk"]*0.08)
    final_sc=int(round(_fr))
