...
if st.button("점수 계산 / 업데이트", use_container_width=True):
    try:
        ...
        progress.empty()
        final_total = progress_total["value"] or candidate_limit
        st.success(f"TOP10 계산 완료: {len(result)}/{final_total}")
...
def run_once():
    ...
    section_rows = [
        ...
        {"구분": "돌파 점수", "점수": score_result.get("Breakout", 0), "설명": "장대양봉, 고점 돌파, 볼린저/거래량 동반"},
