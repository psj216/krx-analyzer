...
def run_once():
    ...
    with pos_col:
        st.markdown("**긍정 요인**")
        positives = summary.get("positive_factors") or score_result.get("PositiveFactors") or ["뚜렷한 우위 요인 없음"]
        for item in positives[:6]:
            st.markdown(f"- {item}")
    with risk_col:
        st.markdown("**주의 요인**")
        risks = summary.get("warning_factors") or score_result.get("RiskFactors") or ["특이 리스크 없음"]
        for item in risks[:6]:
            st.markdown(f"- {item}")
    ...
    section_rows = [
        ...
        {"구분": "돌파 점수", "점수": breakout_display, "설명": "거래량, 캔들 품질, 윗꼬리, ATR 기반 건강한 돌파성"},
    ...
    trend_reasons = list(reasons.get("trend") or tech.get("reasons_trend", []))
    momentum_reasons = list(reasons.get("momentum") or tech.get("reasons_momentum", []))
    ...
    with st.expander("유형별 점수 상세", expanded=False):
        ...
        st.write(_summary_text(score_result, "breakout"))
        st.markdown("**Volume**")
        st.write(_summary_text(score_result, "volume"))
        st.markdown("**Ichi**")
        st.write(_summary_text(score_result, "ichi"))
        st.markdown("**Risk**")
        st.write(_summary_text(score_result, "risk"))

        st.markdown("**상세 근거 리스트**")
        if trend_reasons or momentum_reasons or breakout_reasons:
            for label, items in [("추세", trend_reasons), ("모멘텀", momentum_reasons), ("Breakout", breakout_reasons)]:
                if items:
                    st.markdown(f"**{label} 근거**")
                    for item in items:
                        st.markdown(f"- {item}")
        elif tech.get("details"):
            st.markdown("**보조 상세**")
            for item in tech.get("details", []):
                st.markdown(f"- {item}")
