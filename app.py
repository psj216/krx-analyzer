...
if st.button("점수 계산 / 업데이트", use_container_width=True):
    try:
        ...
        excluded = {k: int(v) for k, v in stats.get("excluded", {}).items() if int(v)}
        msg = f"TOP10 계산 완료: 점수 산출 {scored_count}개 / 후보 {final_total}개"
        if excluded:
            msg += " | 제외 " + ", ".join(f"{v}개: {k}" for k, v in excluded.items())
        st.success(msg)
...
if isinstance(topdf, pd.DataFrame) and not topdf.empty:
    ...
    cols += ["Trend", "Momentum", "Flow", "Vol", "IchiSc", "Overheat", "LiqRisk", "Close"]
    cols = [c for c in cols if c in show.columns]
    rename = {"Vol": "Vol", "IchiSc": "IchiSc"}
...
def run_once():
    ...
    m4.metric("Flow", flow_display)
    m5.metric("Vol / Ichi", f"{vol_display} / {ichi_display}")
    ...
    section_rows = [
        {"구분": "기본점수", "점수": f"{score_result.get('BaseScore', score_value):.2f}", "설명": "5요소 가중합 기준 기본 점수"},
        {"구분": "과열 패널티", "점수": f"-{score_result.get('OverheatPenalty', 0.0):.2f}", "설명": "과열 위험 점수에 따른 차감"},
        {"구분": "유동성 패널티", "점수": f"-{score_result.get('LiqPenalty', 0.0):.2f}", "설명": "저유동성 위험 점수에 따른 차감"},
        {"구분": "최종점수", "점수": f"{score_value}", "설명": f"최종 등급: {grade_value}"},
        {"구분": "추세 점수", "점수": score_result.get("Trend", 0), "설명": "중장기 추세 방향과 안정성"},
        {"구분": "모멘텀 점수", "점수": score_result.get("Momentum", 0), "설명": "단기 힘, RSI, MACD, 5일선 흐름"},
