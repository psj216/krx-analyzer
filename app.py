...
def run_once():
    ...
    if debug_score:
        caption_text += f" | ScorerVersion: {score_result.get('ScorerVersion', 'unknown')}"
    st.caption(caption_text)
    with st.expander("상세분석", expanded=False):
        st.markdown("**📈 추세**")
        ...
        breakout_brief = " | ".join(breakout_reasons) if breakout_reasons else "돌파 신호 없음"
        st.markdown("**💥 돌파성**")
        ...
        if volume_ratio is not None:
            st.write(f"오늘 거래량 / 20일 평균 = {volume_ratio:.2f}배 → 거래량 점수 {score_result.get('Vol', 50)}")
        else:
            st.write("거래량 데이터 부족 → 거래량 점수 50")

        st.markdown("**☁️ 일목**")
        st.write(_summary_text(score_result, "ichi"))

        st.markdown("**💰 수급**")
        st.write(f"수급 상태: {_flow_status_text(flow_detail.get('status', 'N/A'))}")
        st.write(f"최근 5일: {_fmt_score(flow_detail.get('short'))} / 최근 20일: {_fmt_score(flow_detail.get('mid'))} / 지속성: {_fmt_score(flow_detail.get('persist'))}")
        ...
        section_rows = [
            ...
            {"구분": "거래량 점수", "점수": vol_display, "설명": f"오늘/{20}일 평균 {volume_ratio:.2f}배" if volume_ratio is not None else "거래량 비율 계산 불가"},
