...
if isinstance(topdf, pd.DataFrame) and not topdf.empty:
    ...
    cols += ["Trend", "Momentum", "Breakout", "Flow", "Vol", "IchiSc", "Overheat", "LiqRisk", "Close"]
    cols = [c for c in cols if c in show.columns]
    rename = {"Vol": "Volume", "IchiSc": "Ichi"}
...
def run_once():
    ...
    ichi_display = f"{_fmt_score(score_result.get('IchiSc'))}/25"
    ...
    st.markdown("**한줄판단**")
    st.write(score_result.get("Judgment", "판단 근거가 부족합니다."))
    pos_col, risk_col = st.columns(2)
    with pos_col:
        st.markdown("**긍정 요인**")
        positives = score_result.get("PositiveFactors") or ["뚜렷한 우위 요인 없음"]
        for item in positives[:6]:
            st.markdown(f"- {item}")
    with risk_col:
        st.markdown("**주의 요인**")
        risks = score_result.get("RiskFactors") or ["특이 리스크 없음"]
        for item in risks[:6]:
            st.markdown(f"- {item}")

    warn_lines = []
    if overheat_value >= 50:
        warn_lines.append(f"과열 경고: RSI/볼린저/윗꼬리 기준 추격 주의 ({overheat_value})")
    if liq_risk_value >= 40:
        warn_lines.append(f"유동성 경고: 평균 거래대금 {_fmt_eok(score_result.get('AvgTurnover20'))} 수준 확인 필요 ({liq_risk_value})")
    ...
    section_rows = [
        {"구분": "추세 점수", "점수": score_result.get("Trend", 0), "설명": "중장기 추세 방향과 안정성"},
        {"구분": "모멘텀 점수", "점수": score_result.get("Momentum", 0), "설명": "5일선, RSI, MACD 흐름, 단기 이격"},
        {"구분": "돌파 점수", "점수": breakout_display, "설명": "최근 고점 돌파, 장대양봉, 장악형 캔들, 볼린저 상단 돌파"},
        ...
        {"구분": "거래량 점수", "점수": vol_display, "설명": f"오늘 거래량/20일 평균 {score_result.get('VolumeRatio'):.2f}배" if score_result.get("VolumeRatio") is not None else "거래량 비율 계산 불가"},
        {"구분": "일목 점수", "점수": ichi_display, "설명": "전환선/기준선, 구름 위치, 후행스팬 확인"},
        {"구분": "과열 패널티", "점수": f"-{score_result.get('OverheatPenalty', 0.0):.2f}", "설명": f"과열 위험 {overheat_value}"},
        {"구분": "유동성 패널티", "점수": f"-{score_result.get('LiqPenalty', 0.0):.2f}", "설명": f"유동성 위험 {liq_risk_value}"},
    ]
    breakout_reasons = list(tech.get("reasons_breakout", []))
    breakout_keywords = ("돌파", "장대양봉", "장악형", "볼린저", "거래량")
    momentum_reasons = [
        item for item in tech.get("reasons_momentum", [])
        if not any(keyword in str(item) for keyword in breakout_keywords)
    ]
    ...
    with st.expander("유형별 점수 상세", expanded=False):
        ...
        st.markdown("**추세**")
        st.write(score_result.get("TrendSummary", "추세 근거 부족"))
        st.markdown("**모멘텀**")
        st.write(score_result.get("MomentumSummary", "모멘텀 근거 부족"))
        st.markdown("**Breakout**")
        st.write(score_result.get("BreakoutSummary", "Breakout 근거 부족"))
        st.markdown("**Volume**")
        st.write(score_result.get("VolumeSummary", "거래량 근거 부족"))
        st.markdown("**Ichi**")
        st.write(score_result.get("IchiSummary", "일목 근거 부족"))
        st.markdown("**Risk**")
        st.write(score_result.get("RiskSummary", "리스크 근거 부족"))

        st.markdown("**상세 근거 리스트**")
        for item in tech.get("reasons_trend", []):
            st.markdown(f"- {item}")
        for item in momentum_reasons:
            st.markdown(f"- {item}")
        for item in breakout_reasons:
            st.markdown(f"- {item}")

        st.markdown("**수급 점수**")
        st.markdown(f"- 수급 상태: {_flow_status_text(flow_detail.get('status', 'N/A'))}")
        st.markdown(f"- 최근 5일 수급 점수: {_fmt_score(flow_detail.get('short'))}")
        st.markdown(f"- 최근 20일 수급 점수: {_fmt_score(flow_detail.get('mid'))}")
        st.markdown(f"- 외국인/기관 지속성 점수: {_fmt_score(flow_detail.get('persist'))}")

        st.markdown("**거래량 점수**")
        volume_ratio = score_result.get("VolumeRatio")
        if volume_ratio is not None:
            st.markdown(f"- 오늘 거래량이 20일 평균 대비 {volume_ratio:.2f}배로 반영됨 → 거래량 점수 {score_result.get('Vol', 50)}")
        else:
            st.markdown("- 거래량 데이터가 부족해 기본 점수 처리")

        st.markdown("**일목 점수**")
        for item in score_result.get("IchiDetail", []):
            st.markdown(f"- {item}")

        st.markdown("**보조 경고**")
        if overheat_value >= 85:
            st.markdown(f"- 과열 위험으로 점수 차감이 크게 반영되었습니다. (-{score_result.get('OverheatPenalty', 0.0):.2f})")
        if warn_lines:
            for line in warn_lines:
                st.markdown(f"- {line}")
        else:
            st.markdown("- 현재 구간에서는 별도 과열/유동성 경고가 없습니다.")
