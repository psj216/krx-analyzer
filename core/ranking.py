...
def rank_top_scores(
    ...
    def process_one(item):
        ...
        if df is None or df.empty:
            return None
        return compute_score(code, name, market, df, lookback, regime, include_flow=False)
...
def enrich_flow(result_df: pd.DataFrame, regime: str = "sideways", top_n: int = 50) -> pd.DataFrame:
    if result_df is None or result_df.empty:
        return result_df
    required = {"TrendRaw", "MomentumRaw", "IchiSc", "Vol", "Overheat", "LiqRisk"}
    ...
    with ThreadPoolExecutor(max_workers=3) as ex:
        futures = {ex.submit(_fetch_flow, c): c for c in top_codes}
        try:
            for fut in as_completed(futures, timeout=max(top_n, 1) * 6 + 30):
                try:
                ...
                if not mask.any():
                    continue
                idx = df[mask].index[0]
                tr_raw = float(df.loc[idx, "TrendRaw"])
                mo_raw = float(df.loc[idx, "MomentumRaw"])
                vol = float(df.loc[idx, "Vol"])
                ichi_sc = float(df.loc[idx, "IchiSc"])
                overheat = float(df.loc[idx, "Overheat"])
                liq_risk = float(df.loc[idx, "LiqRisk"])
                fr = (
                    w["trend"] * tr_raw
                    + w["momentum"] * mo_raw
                    + w["flow"] * flow
                ...
                fr = max(0.0, fr - overheat * 0.12 - liq_risk * 0.08)
                df.loc[idx, "Flow"] = flow
                df.loc[idx, "Score"] = int(round(fr))
                df.loc[idx, "ScoreRaw"] = float(fr)
        except FuturesTimeout:
            pass

    return df.sort_values("ScoreRaw", ascending=False).reset_index(drop=True)
