...
from core.scorer import MIN_AVG_TURNOVER_20, MIN_PRICE, compute_score
from core.utils import KR_TZ

KOSDAQ_TOP_N_DEFAULT = 1000


def _load_symbol_df(row):
    ...
    try:
        ...
        if df is not None and not df.empty:
            return {"Code": code, "Name": name, "Market": market, "DF": df}
    except Exception:
        pass
    return {"Code": code, "Name": name, "Market": market, "DF": None}
...
def _select_symbols_for_ranking(
    ...
    selected = []
    rejected = {"no_data": 0, "too_short": 0, "halted": 0, "low_price": 0, "low_turnover": 0, "exc": 0}
    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        ...
        try:
            for fut in as_completed(futures, timeout=max(len(kosdaq_rows), 1) * 5 + 180):
                try:
                    ...
                    rejected["exc"] = rejected.get("exc", 0) + 1
    ...
    if kosdaq_candidates:
        ...
        kosdaq_limit = min(limit if limit and limit > 0 else KOSDAQ_TOP_N_DEFAULT, KOSDAQ_TOP_N_DEFAULT)
        kosdaq_candidates = kosdaq_df.head(kosdaq_limit).to_dict("records")

    with ThreadPoolExecutor(max_workers=8) as ex:
        futures = {ex.submit(_load_symbol_df, row): row for row in kospi_rows}
        try:
            for fut in as_completed(futures, timeout=max(len(kospi_rows), 1) * 5 + 180):
                try:
                    selected.append(fut.result(timeout=FETCH_TIMEOUT + 3))
                except Exception:
                    row = futures[fut]
                    selected.append(
                        {
            ...
            for row in kospi_rows:
                code = str(row["Code"]).zfill(6)
                if not any(item["Code"] == code for item in selected):
                    selected.append(
                        {
                            "Code": code,
                            "Name": str(row["Name"]),
...
def rank_top_scores(
    ...
    return (
        pd.DataFrame(rows)
        .sort_values(
            ["ScoreRaw", "Score", "Trend", "Momentum", "Breakout", "Vol", "IchiSc", "Name"],
