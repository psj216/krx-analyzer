...
def fetch_prices_with_retry(code: str, market: str, retry_count: int = 2) -> pd.DataFrame | None:
    ...
    for attempt in range(attempts):
        try:
            df = fetch_prices(code, market, allow_fallback=False)
...
def _select_symbols_for_ranking(
    ...
    return selected, {"price_data_missing": 0, "insufficient_length": 0, "score_failed": 0}
...
def rank_top_scores(
    ...
    excluded = {"price_data_missing": 0, "insufficient_length": 0, "score_failed": 0}
    ...
    def process_one(item):
        ...
        if not _has_required_price_columns(df):
            return None, "price_data_missing"
        if len(df) < MIN_SCORE_ROWS:
