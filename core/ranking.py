...
def fetch_prices_with_retry(code: str, market: str, max_retries: int = 5) -> pd.DataFrame | None:
    last_df = None
    ...
    return last_df if _has_required_price_columns(last_df) else None
...
def rank_top_scores(
    ...
    excluded = {"가격 데이터 없음": 0, "점수 계산 실패": 0}
    ...
    for i, item in enumerate(candidates, start=1):
        ...
        df = None
        try:
            df = fetch_prices_with_retry(code, market)
            if not _has_required_price_columns(df):
                excluded["가격 데이터 없음"] += 1
                continue
