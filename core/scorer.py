...
def score_technical_practical(df: pd.DataFrame, signals: dict, fib: dict) -> dict:
    ...
    bull_engulfing = candle_bull_engulfing(df)
    ...
    if signals.get("bb_breakout_up"):
        trend -= 3
        reasons_trend.append("볼린저 상단 과열 가능성 → -3")
    ...
    if not pd.isna(rsi):
        if 55 <= rsi <= 65:
            ...
            momentum -= 4
            reasons_momentum.append("RSI 75↑ → -4 / Overheat 반영")
            risks.append("RSI 75 이상")
        elif rsi >= 80:
            momentum -= 6
            reasons_momentum.append("RSI 80↑ → -6 / Overheat 반영")
    ...
    if long_bull and vol_ratio >= 1.2:
        momentum += 5
        reasons_momentum.append("장대양봉+거래량 → +5")
        positives.append("장대양봉+거래량")
    if bull_engulfing and vol_ratio >= 1.2:
        momentum += 4
        reasons_momentum.append("상승 장악형+거래량 → +4")
        positives.append("상승 장악형+거래량")
    if lower_tail >= 0.35 and close >= open_:
        momentum += 3
        reasons_momentum.append("아래꼬리 반등형 → +3")
        positives.append("아래꼬리 반등형")
    if upper_tail >= 0.50:
        momentum -= 6
        reasons_momentum.append("윗꼬리 50%↑ → -6")
        risks.append("윗꼬리 50% 이상")
    elif upper_tail >= 0.40:
        momentum -= 4
        reasons_momentum.append("윗꼬리 40%↑ → -4")
        risks.append("윗꼬리 40% 이상")
    if prior_gain_atr > 1.0:
        momentum -= 3
        reasons_momentum.append("전일 급등 ATR초과 → -3")
        risks.append("전일 급등 ATR 초과")
    if day_range_atr > 2.0:
        momentum -= 5
        reasons_momentum.append("당일 변동폭 ATR 과도 → -5")
        risks.append("ATR 대비 변동폭 과도")

    # Breakout: bucket-based (20/40/60/80/100)
    breakout_pts = 0
    if 1.2 <= vol_ratio < 5.0:
        breakout_pts += 1
        reasons_breakout.append("거래량 1.2~5배 → +1")
    if len(df) >= 21 and close > _as_float(df["High"].iloc[-21:-1].max(), close * 2):
        breakout_pts += 1
        reasons_breakout.append("최근 고점 돌파 참고 → +1")
    if long_bull or bull_engulfing:
        breakout_pts += 1
        reasons_breakout.append("장대양봉 또는 상승 장악형 → +1")
    close_at_high = close >= open_ and upper_tail <= 0.10
    lower_tail_bounce = lower_tail >= 0.35 and close >= open_
    if close_at_high or lower_tail_bounce:
        breakout_pts += 1
        reasons_breakout.append("종가 고가권 마감 → +1" if close_at_high else "아래꼬리 반등형 → +1")
    ...
    breakout_cap = 100
    if upper_tail >= 0.50:
        breakout_cap = min(breakout_cap, 20)
        reasons_breakout.append("윗꼬리 50%↑로 최대 20 제한")
    elif upper_tail >= 0.40:
        breakout_cap = min(breakout_cap, 40)
        reasons_breakout.append("윗꼬리 40%↑로 최대 40 제한")
    if signals.get("bb_breakout_up"):
        breakout_cap = min(breakout_cap, 60)
        reasons_breakout.append("볼린저 상단 과열로 최대 60 제한")
    if prior_gain_atr > 1.0:
        breakout_cap = min(breakout_cap, 60)
        reasons_breakout.append("전일 급등 ATR초과로 최대 60 제한")
    if day_range_atr > 2.0:
        breakout_cap = min(breakout_cap, 40)
        reasons_breakout.append("당일 변동폭 ATR 과도로 최대 40 제한")
    if vol_ratio >= 8.0:
        breakout_cap = min(breakout_cap, 40)
        reasons_breakout.append("거래량 8배↑로 최대 40 제한")
    elif vol_ratio >= 5.0:
        breakout_cap = min(breakout_cap, 60)
        reasons_breakout.append("거래량 5~8배로 최대 60 제한")

    breakout = float(min(breakout, breakout_cap))
    reasons_breakout.append(f"→ Breakout {int(breakout)}")
    ...
    if signals.get("bb_breakout_up"):
        overheat += 15
        overheat_reasons.append("볼린저 상단 과열 가능성")
    if vol_ratio >= 8:
        overheat += 25
    ...
    return {
        ...
        "ScorerVersion": "bucket_breakout_v2",
    }
