...
def score_technical(df: pd.DataFrame, signals: dict, fib: dict) -> dict:
    ...
    if not pd.isna(sma120):
        ...
        trend += delta
        reasons_trend.append(f"SMA120 기준 {'상방 유지' if delta > 0 else '하방 위치'} ({delta:+.0f})")
    if n >= 2 and not pd.isna(sma20):
        ...
        if prev_sma20:
            ...
            trend += delta
            reasons_trend.append(f"SMA20 기울기 {slope20*100:.2f}% ({delta:+.0f})")
    if n >= 2 and not pd.isna(sma60):
        ...
        if prev_sma60:
            ...
            trend += delta
            reasons_trend.append(f"SMA60 기울기 {slope60*100:.2f}% ({delta:+.0f})")
    ...
    if not pd.isna(sma5) and sma5:
        ...
        if above5:
            momentum += 4.0
            reasons_momentum.append("5일선 상방 유지 (+4)")
        if n >= 2:
            ...
            if above5 and rising5:
                momentum += 2.0
                reasons_momentum.append("5일선 상방 + 우상향 콤보 (+2)")
            if slope5 > 0.015:
                momentum -= 2.0
                reasons_momentum.append("5일선 기울기 과열 (-2)")
        if gap5_pct >= 95:
            momentum -= 4.0
            reasons_momentum.append("5일선 이격 과열 상위 5% (-4)")
        elif gap5_pct >= 90:
            momentum -= 2.0
            reasons_momentum.append("5일선 이격 과열 상위 10% (-2)")
        elif 0 < gap5_now <= 0.03:
            momentum += 3.0
            reasons_momentum.append("5일선 이격 0~3% 적정 (+3)")

    if rsi_now is not None:
        if 50 < rsi_now <= 55:
            momentum += 4.0
            reasons_momentum.append(f"RSI {rsi_now:.1f} 초입 모멘텀 (+4)")
        elif 55 < rsi_now <= 65:
            momentum += 3.0
            reasons_momentum.append(f"RSI {rsi_now:.1f} 우호 구간 (+3)")
        if 70 <= rsi_now < 75:
            momentum -= 1.0
            reasons_momentum.append(f"RSI {rsi_now:.1f} 과열 경고 (-1)")
        elif 75 <= rsi_now < 80:
            momentum -= 3.0
            reasons_momentum.append(f"RSI {rsi_now:.1f} 과열 부담 (-3)")
        elif rsi_now >= 80:
            momentum -= 5.0
            reasons_momentum.append(f"RSI {rsi_now:.1f} 과열 강함 (-5)")
    ...
    if n >= 3 and "RSI14" in df.columns and rsi_now is not None:
        ...
        if rs1 > 0 and rs2 > 0 and 30 < rsi_now < 65:
            momentum += 4.0
            reasons_momentum.append("RSI 연속 상승 확인 (+4)")
        elif rs1 < 0 and rs2 < 0 and rsi_now > 50:
            momentum -= 3.0
            reasons_momentum.append("RSI 상승 탄력 약화 (-3)")
    ...
    if "MACD_12_26_9" in df.columns and n >= 3:
        ...
        if hist > 0 and hist_prev <= 0:
            momentum += 5.0
            reasons_momentum.append("MACD 상향 전환 초입 (+5)")
        if hist > 0 and hist_prev > 0 and hist > hist_prev:
            if macd_slope1 > 0 and macd_slope1 > macd_slope2:
                ...
                momentum += 3.0
                reasons_momentum.append("MACD 증가 유지 (+3)")
        if hist_prev > 0 and hist < hist_prev:
            momentum -= 4.0
            reasons_momentum.append("MACD 히스토그램 둔화 (-4)")
    ...
    if all(c in df.columns for c in ["BBL_20_2.0", "BBM_20_2.0", "BBU_20_2.0"]) and n >= 2:
        ...
        if width_prev < 0.05 and width_now > width_prev * 1.1:
            momentum += 3.0
            reasons_momentum.append("볼린저 squeeze 확장 시작 (+3)")
        dist_bbu = (close - bbu) / (bbu or 1.0)
        if 0.03 < dist_bbu <= 0.05:
            momentum -= 3.0
            reasons_momentum.append("볼린저 상단 과열 3~5% (-3)")
        if dist_bbu > 0.05:
            momentum -= 6.0
            reasons_momentum.append("볼린저 상단 과열 5% 초과 (-6)")
    ...
    if "Volume" in df.columns:
        ...
        if vol_now < 10_000:
            momentum -= 8.0
            reasons_momentum.append("거래량 1만주 미만 (-8)")
        elif vol_now < 30_000:
            momentum -= 4.0
            reasons_momentum.append("거래량 1~3만주 수준 (-4)")
        if vol_pct <= 20:
            momentum -= 6.0
            reasons_momentum.append("거래량 하위 20% (-6)")
        elif 1.2 <= ratio <= 2.5:
            delta = 5.0 if is_breakout else 3.0
            momentum += delta
            reasons_momentum.append(f"거래량 1.2~2.5배 {'+ 돌파 동반 ' if is_breakout else ''}({delta:+.0f})")
        elif 2.5 < ratio <= 5:
            delta = 4.0 if is_breakout else 1.0
            momentum += delta
            reasons_momentum.append(f"거래량 2.5~5배 {'+ 돌파 동반 ' if is_breakout else ''}({delta:+.0f})")
        elif 5 < ratio <= 8:
            momentum -= 2.0
            reasons_momentum.append("거래량 급증 5~8배 과열 경계 (-2)")
        elif ratio > 8:
            momentum -= 5.0
            reasons_momentum.append("거래량 급증 8배 초과 과열 경계 (-5)")
    ...
    if signals.get("turnover_surge"):
        ...
        if tail_up < 0.3:
            momentum += 3.0
            reasons_momentum.append("거래대금 급증 + 윗꼬리 짧음 (+3)")
        else:
            momentum += 1.0
            reasons_momentum.append("거래대금 급증이나 윗꼬리 부담 존재 (+1)")
    ...
    if close > open_ and body / rng >= 0.6 and tail_up < 0.2:
        ...
        momentum += delta
        reasons_momentum.append(f"몸통 양봉 {'+ 거래량' if vol_pct >= 60 else ''} ({delta:+.0f})")
    if tail_dn >= 0.4 and close >= open_:
        momentum += 3.0
        reasons_momentum.append("아래꼬리 반등 확인 (+3)")
    if tail_up >= 0.4:
        momentum -= 4.0
        reasons_momentum.append("윗꼬리 40% 이상 매물 부담 (-4)")
    if candle_bull_engulfing(df):
        delta = 4.0 if vol_pct >= 60 else 2.0
        momentum += delta
        reasons_momentum.append(f"양봉 감싸기 {'+ 거래량' if vol_pct >= 60 else ''} ({delta:+.0f})")

    if not pd.isna(atr_val) and atr_val > 0 and close > 0:
        atr_pct = atr_val / close
        if abs(day_ret) > atr_pct * 2.5:
            momentum -= 5.0
            reasons_momentum.append("당일 변동성 과도 (-5)")
        elif 0.01 <= day_ret <= atr_pct * 1.5:
            momentum += 2.0
            reasons_momentum.append("당일 상승이 안정 범위 (+2)")
        elif day_ret > atr_pct * 1.5:
            momentum -= 3.0
            reasons_momentum.append("당일 급등 과도 (-3)")
    else:
...
def volume_score(df: pd.DataFrame, days: int = 20) -> int:
    try:
        ...
        if avg_vol <= 0:
            return 50
        return int(max(0, min(100, 50 + 40 * np.tanh(last_vol / avg_vol - 1))))
    except Exception:
        return 50
