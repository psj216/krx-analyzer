"""
core/scorer.py
Five-factor score model used by app and Telegram.
"""
...
def _grade_label(score: float, overheat: float = 0.0) -> str:
    if score >= 72:
        ...
        label = "주의"
    return f"{label} / 과열 주의" if overheat >= 85 else label
...
def compute_score(
    ...
    try:
        ...
        try:
            ...
            signals = {}
        try:
            tech = score_technical(df, signals, fib) or _default_technical()
        except Exception:
            tech = _default_technical()
        for key, value in _default_technical().items():
            tech.setdefault(key, value)
        if tech.get("reasons_breakout"):
            tech["reasons_momentum"] = list(tech.get("reasons_momentum", [])) + [
                f"돌파 신호: {reason}" for reason in tech.get("reasons_breakout", [])
            ]
        flow_score = None
        ...
        if flow_score is None:
            denom = BASE_WEIGHTS["trend"] + BASE_WEIGHTS["momentum"] + BASE_WEIGHTS["volume"] + BASE_WEIGHTS["ichi"]
            weights = {
        ...
        final_raw = base_score - overheat_penalty - liq_penalty
        ...
        return {
            ...
            "Trend": int(round(_as_float(tech.get("trend_score"), _as_float(tech.get("trend_raw"), 50.0)))),
            "Momentum": int(round(_as_float(tech.get("momentum_score"), _as_float(tech.get("momentum_raw"), 50.0)))),
            "Flow": flow_score,
            "Vol": vol_score,
            "IchiSc": ichi_score,
